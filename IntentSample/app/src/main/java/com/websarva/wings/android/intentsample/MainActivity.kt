...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //SimpleAdapterで使用するMutableListオブジェクトを用意。 1-1
        val menuList: MutableList<MutableMap<String, String>> = mutableListOf()
        //「から揚げ定食」のデータを格納するMapオブジェクトの用意とmenuListへのデータ登録。1-2
        var menu = mutableMapOf("name" to "唐揚げ定食", "price" to "800円")
        menuList.add(menu)
        //「ハンバーグ定食」のデータを格納するMapオブジェクトの用意とmenuListへのデータ登録。1-3
        ...
        //SimpleAdapter第4引数from用データの用意。2-1
        val from = arrayOf("name", "price")
        //SimpleAdapter第5引数to用データの用意。2-2
        val to = intArrayOf(android.R.id.text1, android.R.id.text2)
        //SimpleAdapterを生成。2-3
